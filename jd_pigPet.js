...
!(async () => {
  ...
  if (process.env.PIGPETSHARECODE) {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/shufflewzc/updateTeam/main/shareCodes/pigPet.json')
    if (!res) {
      res = await getAuthorShareCode('https://raw.fastgit.org/shufflewzc/updateTeam/main/shareCodes/pigPet.json')
    }
    if (res){
  ...
  console.log(`\n======开始大转盘助力======\n`);
  $.helpId = await getAuthorShareCode('https://raw.fastgit.org/shufflewzc/updateTeam/main/shareCodes/pig.json');
